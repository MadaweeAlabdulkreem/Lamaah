...
from ultralytics.nn.modules.hyperace import *
...
def parse_model(d, ch, verbose=True):
    from ultralytics.nn.modules.acmft3_yolo26 import acmf_TD_P2, acmf_TD_P3, acmf_TD_P4  # acmf-local
    globals().update(dict(acmf_TD_P2=acmf_TD_P2, acmf_TD_P3=acmf_TD_P3, acmf_TD_P4=acmf_TD_P4))  # acmf-globals
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if isinstance(f, int):
                ...
                c1 = sum([ch[i] for i in f])  # نجمع القنوات لكل الطبقات اللي في القائمة
            else:
                raise TypeError(f"Unsupported type for 'from' index f: {type(f)}")
            ...
            c1 = [ch[x] for x in f]   # channels of layers 4,6,8
            args = [c1, *args]       # send them to module
            c2 = args[1]
                    # ===== pbap SUPPORT =====
        elif m in {
            pbap_TD_P4,
            ...
            c1 = [ch[x] for x in f]   # multiple inputs (P2,P3,P4,P5)
            args = [c1, *args]        # pass list of channels to module
            c2 = args[1]              # output channels (64)
            ...
            c2   = ch[f]          
            args = [c2, *args] 
        elif m in {acmf_TD_P2, acmf_TD_P3, acmf_TD_P4}:
            assert isinstance(f, list) and len(f) == 3, (
                f"{m.__name__} requires exactly 3 input indices, got f={f}"
            )
            c1   = [ch[x] for x in f]
            c2   = c1[0] if m is acmf_TD_P2 else c1[1] if m is acmf_TD_P3 else c1[2]
            args = [c1, *args]
        else:
            c2 = ch[f]
