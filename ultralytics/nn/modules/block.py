...
#  Equations implemented (verbatim from paper §Non-semantic sparse

#    (1)  F  = δ( AP( Conv(X) ) )
#    (2)  Q, K, V = SparseQ(X), SparseK(X), SparseV(X)
#    (3)  F′ = Conv( Unsparse( MHSA(Q, K, V) ) )
#    (4)  X′ = Reshape( F ⊗ F′ )
#
...
class MHSA(nn.Module):
    """
    MHSA — Channel-wise Multi-Head Self-Attention.

    Implements the internals of Eq. (2) and the MHSA call in Eq. (3).

    When invoked from svaA the input has already been sparsified, so the
    effective batch size is B·S² and each element attends only within its
    own spatial block — matching the paper: "only tensor blocks labelled
    with the same colour perform self-attention computations."

    Strict paper fidelity:
      • No LayerNorm   — not mentioned in the paper
      • No inner residual — not mentioned in the paper
      • Fused QKV linear  — equivalent to three separate sparse projections
    ...
    def __init__(self, c: int, num_heads: int = 8):
        ...
        assert c % num_heads == 0, (
            f"channels {c} must be divisible by num_heads {num_heads}"
        )
        ...
        # Eq (2): SparseQ, SparseK, SparseV — single fused linear then split.
        # Mathematically identical to three independent projections on the
        # same (sparsified) input.
        self.qkv  = nn.Linear(c, 3 * c, bias=False)

        # Output projection — implicit in any multi-head attention block.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Eq (2): Q, K, V from the same (sparsified) input
        q, k, v = self.qkv(tokens).chunk(3, dim=-1)     # each (B, H·W, C)
        ...
        # Scaled dot-product attention (MHSA)
        attn = (q @ k.transpose(-2, -1)) * self.sc      # (B, nh, H·W, H·W)
        attn = attn.softmax(dim=-1)
        out  = attn @ v                                  # (B, nh, H·W, hd)

        # Merge heads: (B, nh, H·W, hd) → (B, H·W, C)
        out = out.transpose(1, 2).reshape(B, -1, C)

        # Output projection (no residual — not in paper)
...
# ─────────────────────────────────────────────────────────────────────────────
class svaA(nn.Module):
    """
    Non-Semantic Sparse Attention  (SMA-YOLO, Zhou et al. 2025).

    The key design choice — Fig. 2 of the paper shows an interleaved
    ...
    S         : sparse coefficient (default 2).
                H % S == 0  and  W % S == 0  must hold.
                S = 1  →  single block, global attention (paper default fallback).
    num_heads : attention heads inside MHSA (default 8)
    """
    ...
    def __init__(self, c: int, S: int = 2, num_heads: int = 8):
        ...
        # Eq (2–3) branch: MHSA on sparsified features
        self.ca = MHSA(c, num_heads)

        # Eq (3): Conv in  F′ = Conv(Unsparse(MHSA(…)))
        self.b2 = nn.Conv2d(c, c, kernel_size=1, bias=False)

    # ── Sparsify ─────────────────────────────────────────────────────────────
    def _sp(self, x: torch.Tensor, S: int) -> torch.Tensor:
        """
        Interleaved / stride-S partition of (B, C, H, W) into S² blocks.
        Output shape: (B·S², C, H/S, W/S).

        The reshape order  (B, C, H/S, S, W/S, S)  groups every S-th row
        together (stride S), producing the checkerboard pattern shown in
        Fig. 2 of the paper.  Each resulting block samples the full spatial
        extent of the feature map at stride S, so self-attention within a
        block captures non-local, non-semantic structure.

        Compare with the WRONG order  (B, C, S, H/S, S, W/S)  which groups
        consecutive rows and yields contiguous quadrant blocks — each block
        would then attend only within a local corner.

        Verified pixel-exact against Fig. 2 for a 4×4 grid with S=2:
          Block 0 → pixels {0,2,8,10}   (even rows, even cols)
          Block 1 → pixels {1,3,9,11}   (even rows, odd  cols)
          Block 2 → pixels {4,6,12,14}  (odd  rows, even cols)
          Block 3 → pixels {5,7,13,15}  (odd  rows, odd  cols)
        """
        B, C, H, W = x.shape
        return (
    ...
    def _unsp(self, x: torch.Tensor, B: int, H: int, W: int, S: int) -> torch.Tensor:
        """
        Exact inverse of _sp.  Restores (B·S², C, H/S, W/S) → (B, C, H, W).

        Paper: "Unsparse indicates restoring the shape prior to
        sparsification."

        The permute (0,3,4,1,5,2) is the mathematical inverse of the
        forward permute (0,3,5,1,2,4) used in _sp.
        Round-trip error verified == 0.0 for all tested shapes.
        """
        C = x.shape[1]
        return (
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x : (B, C, H, W)   —  H % S == 0  and  W % S == 0  required.
        returns X′ : (B, C, H, W)
        """
        B, C, H, W = x.shape
        S = self.S

        # Paper: "When the sparse coefficient S is 1, it is equivalent to
        # a single block, performing global attention."
        # During Ultralytics multi-scale training some feature maps have
        # non-divisible spatial dims (e.g. 12×21).  Fall back to S=1
        # (global attention) for those cases — this is the paper's own
        # documented fallback, not a deviation from it.
        if H % S != 0 or W % S != 0:
            S = 1

        # ── Eq (1): Branch 1 — channel-wise attention weights ─────────────
        #   Conv(X) → AP(·) → δ(·)
        #   Result: (B, C, 1, 1) — broadcasts over H and W in Eq. (4)
        F_w = torch.sigmoid(
            F.adaptive_avg_pool2d(self.b1(x), output_size=1)
        )                                                  # (B, C, 1, 1)
        ...
        #   MHSA within each block              →  (B·S², C, H/S, W/S)
        #   Unsparse                               →  (B, C, H, W)
        #   Conv                                   →  F′ (B, C, H, W)
        xs  = self._sp(x, S)                               # (B·S², C, H/S, W/S)
        F_p = self.b2(
            self._unsp(self.ca(xs), B, H, W, S)
        )                                                  # (B, C, H, W)

        # ── Eq (4): X′ = Reshape( F ⊗ F′ ) ───────────────────────────────
        #   Reshape() is fulfilled by _unsp above — both tensors are already
        #   in (B, C, H, W) at this point.
